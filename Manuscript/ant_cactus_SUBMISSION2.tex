...
\tom{Furthermore, \textit{C. opuntiae}-tended plants had fewer seeds per fruit (115.0[79.5--165.5] seeds) than vacant (147.2[114.1--189.9] seeds) or \textit{L. apiculatum}-tended plants (142.4[100.7--200.2] seeds), likely due to pollinator interference \citep{Ohm2014}.}{I think a reviewer asked to see this result, so be sure to reference the figure in the appendix and flag this as a revision in the response letter. Also, the figure legend should be updated to indicate that we are looking at posterior probability distributions, which some readers will not understand if you don't explicitly state.}
...
For example, our previous study that focused only on reproductive vital rates suggested that \textit{C. opuntiae} has overall parasitic fitness effects because activity of this species within tree cholla flowers can deter pollinators and reduce seed \tom{set}{Reference the appendix figure here.} \citep{Ohm2014}. 
...
\tom{Surveys of herbivore damage from our long-term data are consistent with protective benefits of ant visitation (Figure \ref{app:herb}), and more explicit inclusion of plant-herbivore-defender interactions could be a valuable future direction. }{The reviews brought up that the herbivory figure in the appendix is not adequately discussed or contextualized, and I think that issue remains to be addressed.}

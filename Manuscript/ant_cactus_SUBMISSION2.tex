...
Here we asked: how do partner diversity and turnover influence host demography and long-term fitness, and which mechanisms explain these effects?}
...
While traditionally framed as pairwise (+/+) interactions, mutualisms are often embedded within multispecies communities in which a focal species interacts with multiple partner species.
...
\revise{Second, even within a mutualist guild, the benefits conferred by alternative partner species can vary by method, efficiency, or timing \citep{Stachowicz2005,Bronstein2006,Stanton2003}. \linelabel{res:L53}\tom{}{There are two different Stachowicz references for the same paper -- be sure to clean this up.}
...
Critically, portfolio effects require asynchronous fluctuations among partners, such that declines in one partner's benefits are offset by others, stabilizing the focal mutualist's fitness over time \citep{Loreau1999, Tilman1994, Hautier2014}.
%This concept is well developed in biodiversity–ecosystem functioning literature, but has rarely been applied to mutualisms in which partner diversity is realized through turnover rather than coexistence.
While portfolio effects have been well studied in plant and animal communities, their role in mutualisms —- particularly where partner diversity is sequential rather than simultaneous —- is less explored \citep{Ushio2020}.}

%% Partner turnover
%Partner diversity can affect a focal mutualist differently depending on whether partners are present simultaneously or sequentially (i.e., via partner turnover) \citep{Djieto-Lordon2005, Ness2006, Bruna2014, Barrett2015, Ushio2020, Dattilo2014}. 
%In simultaneous associations, interference between partners may reduce the net benefits received by a focal mutualist \citep{Kiers2003,Batstone2018, Tgaard2015, Wulff2008}. \linelabel{res:L63}
%Exclusive, sequential associations can reduce antagonistic partner-partner interactions \citep{Wolfin2023,Stachowicz2005_2} and enhance mutualism stability \cite{Johnson2019}. 

%%What if we skip the contrast of simultaneous vs sequential, since it is really the sequential ideas that set up our study?
\revise{For exclusive, sequential associations, the timing, frequency, and directionality of partner turnover can influence the net fitness benefits to a focal mutualist, particularly if turnover allows accumulation of complementary benefits or buffers environmental variability via portfolio effects \citep{Sachs2004,Fiala1994, Ushio2020}. \linelabel{res:L63}
\tom{Turnover can happen at different timescales, from minutes to years \citep{Oliveira1999,Horvitz1986}.}{any more recent examples we could cite here?} 
...
\revise{Previous studies have investigated how ant partner diversity affects plant fitness \citep{Palmer2010,Fiala1994,Gaume1998,Dattilo2014,Ludka2015}, however little is known about the combined effects of partner identity, directional partner turnover and temporal stochasticity, particularly because long-term demographic studies on ant-plant interactions are \tom{exceedingly rare}{Any other examples we can cite here? How long was the Palmer Acacia study?}. 

Long-term demographic data are essential to understanding the fitness consequences of multi-species mutualisms for several reasons. \linelabel{res:long-term-data}
...
\tom{Second, certain effects of partner diversity are fluctuation-dependent (i.e., portfolio effect) and would only be detectable in the context of temporally stochastic environments in which alternative partners may fluctuate asynchronously. 
Quantifying contributions from the portfolio effect therefore requires long-term data on partner (a)synchrony across inter-annual fluctuations. }{This might be a place to more thoroughly address R2's comment about what evidence for portfolio effect would look like.}
...
\tom{Most studies evaluate short-term fitness proxies like seed set or herbivory at a single life stage \citep{Palmer2010, Bruna2014, Trager2010}, while few trace demographic effects across multiple stages.}{I like this sentence but it is unclear whether you are using the citations as examples of studies that only consider single life stages, or the opposite. This would be good to clarify, since you don't want to mis-cite people.}
...
For all plants, in May or early June of each year we recorded plant survival since the last survey (if the plant had any green flesh it was considered alive) and, for survivors, we recorded height (cm), maximum crown width (cm), and crown width perpendicular to the maximum (cm) (sections with green were measured for size).
...
\tom{In total, the data set includes a total of 9,787 observations of 1,141 individuals across 15 complete transition years (spanning May/June of year $t$ to May/June of year $t+1$) \citep{DataCholla}. }{Update with new data package citation, and also update data availability statement.}
...
The tree cholla population is comprised of plants characterized by their size, which can take any continuous value between the lower and upper limits, and ant visitation status, which can take one of the four discrete levels described above. 
...
Seeds in the two-year seed bank are assumed to either germinate with probability $\gamma_2$ or die.  \linelabel{res:seed_prod}
...
Additional reproductive parameters for probability of entry to the seed bank, germination rates, and recruit size were estimated following methods described in Appendix A. 
...
\revise{\linelabel{res:portfolio_methods}In Appendix D, we explore additional scenarios that vary the correlation of ant-specific fluctuations, to determine if contributions from the portfolio effect are constrained by high correlations seen in the long-term data (Results).}
...
Furthermore, \textit{C. opuntiae}-tended plants had fewer seeds per fruit (115.0[79.5--165.5] seeds) than vacant (147.2[114.1--189.9] seeds) or \textit{L. apiculatum}-tended plants (142.4[100.7--200.2] seeds), likely due to pollinator interference \citep{Ohm2014}. \linelabel{res:viability}
\ali{}{I am not sure where in the appendix you want this to be referenced?}
...
\tom{Inter-annual fluctuations in demographic rates (estimated as statistical random effects) were generally positively correlated between ant states, limiting the potential for benefits of diversity through the portfolio effect (Figure \ref{fig:Annual_Ant}).}{Be sure to update this figure and flag it as a revision in the response letter.} 
...
Certain partner pair / vital rate combinations were effectively independent in their yearly fluctuations (\tom{such as survival of \textit{C. opuntiae} and Other-tended plants}{Give the correlation coefficient.}) while others were almost completely synchronous (\tom{such as growth of Vacant and \textit{L. apiculatum}-tended plants}{Give the correlation coefficient.}). 
...
Using the simulations where all ants had equal frequencies across sizes (further explained in Appendix C), we found the same fitness patterns as described above. \linelabel{res:appendixc}
...
\revise{\linelabel{res:portfolio_results}In Appendix D, we show that contributions from the portfolio effect were constrained by positively correlated fluctuations between ant states (Figure \ref{fig:Annual_Ant}). 
In scenarios in which partner fluctuations were uncorrelated or, especially, negatively correlated, the non-synchronous model exhibited a stronger benefit of diversity than the synchronous model (Figures), as expected under the portfolio effect hypothesis.}
...
% Portfolio Effect
\tom{When}{I think you will want to expand this paragraph to discuss the new results.} partners exhibit different reactions to varying environments, interacting with a diverse portfolio of partners can lead to more consistent benefits across time \citep{Batstone2018}.
...
\tom{Surveys of herbivore damage from our long-term data are consistent with protective benefits of ant visitation (Figure \ref{app:herb}), and more explicit inclusion of plant-herbivore-defender interactions could be a valuable future direction. }{The reviews brought up that the herbivory figure in the appendix is not adequately discussed or contextualized, and I think that issue remains to be addressed.} \linelabel{res:herbivory}
...
\tom{}{Never forget to thank reviewers in the acknowledgments of resubmissions!}
...
To further evaluate the potential for a portfolio effect in this system, we conducted an additional analysis in which we artificially increased the degree of difference among ant partners. 
...
For each of these scenarios, we quantified the effect of partner diversity as the difference between projected population growth with and without partner diversity ($\lambda_S^4 - \lambda_S^0$), and compared results under synchronous versus non-synchronous environmental variation. 
Both scenarios resulted in greater benefits of partner diversity under Non-synchronous conditions, though the evidence was much stronger when the ants were anti-correlated. 
...
\begin{figure}
	...
	\caption{The mean estimations of $\lambda_{S}^4-\lambda_{S}^0$ for the non-synchronous and synchronous models are shown in green and pink respectively. }
	\label{app:portfolio}
\end{figure}

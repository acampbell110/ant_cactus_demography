...
\author{
	...
	Tom E.X. Miller$^{1}$ (ORCID: 0000-0003-3208-6067)
}
...
\begin{document}
	...
	\noindent
	$^{1}$ Program in Ecology and Evolutionary Biology, Department of BioSciences, Rice University, Houston, Texas 77005
	
	\noindent
	$^{\dagger}$ Corresponding author: ali.campbell@berkeley.edu
	
	
	\bigskip
	
	\textit{Keywords}: Integral Projection Model, \textit{Cylindropuntia imbricata}, multi-species mutualism, complementarity, sampling effect, portfolio effect
...
In sequential mutualisms, partner identity can change over time due to environmental variability or competitive displacement. 
...
Mutualisms, species interactions where all participants receive net benefits, are key components of ecological communities and can enhance the fitness and population growth rates of the partners involved. 
...
Partner diversity can arise either from simultaneous interactions or sequential ones, where individual hosts experience turnover in partner identity over time \citep{Djieto-Lordon2005, Ness2006}.
Understanding how partner diversity and turnover affect the overall fitness benefits of mutualism is an open question, especially in the context of environmental variability, where different partners may confer benefits unevenly across space or time  \citep{rogers2014, Thibaut2012, Frederickson2005, Palmer2010}. 

%% Look at mechanisms which show how diversity impacts demography
There are multiple mechanisms by which partner diversity can influence the net benefits accrued by a focal mutualist, mirroring the mechanisms by which, at a larger scale of organization, biodiversity can influence ecosystem function \citep{Barrett2015,Ushio2020,Winfree2020}. 
First, when there is a consistent ranking of best to worst mutualists, a more diverse sample of the partner community may be more likely to include the best partner \citep{Frederickson2013}. 
...
Second, even within a mutualist guild, the benefits conferred by alternative partner species can vary by method, efficiency, or timing \citep{Stachowicz2005,Bronstein2006,Stanton2003}. 
...
In the context of temporally fluctuating environments, the portfolio effect implies that inter-annual variation in partner identity could stabilize overall plant fitness across years. 
For such a portfolio effect to operate, two conditions must be met: (i) partner species must differ in the timing or magnitude of their responses to environmental variation (i.e., their benefits fluctuate asynchronously), and (ii) the combined effect of these fluctuations must result in increased net benefit to the host across years relative to the net benefit expected if all partners responded in parallel \citep{Loreau1999, Tilman1994, Hautier2014}. 
...
For exclusive, sequential associations, the timing, frequency, and directionality of partner turnover can influence the net fitness benefits to a focal mutualist, particularly if turnover allows accumulation of complementary benefits or buffers environmental variability via portfolio effects \citep{Sachs2004,Fiala1994, Ushio2020}. 
...
Ant partners can vary in their ability to deter herbivores \citep{Bruna2014} and visitation by low quality ant partners can prevent visitation by higher quality partners, causing a reduction in relative fitness through  opportunity costs \citep{Fraser2001, Frederickson2005}. 
...
Previous studies have investigated how ant partner diversity affects plant fitness \citep{Palmer2010,Fiala1994,Gaume1998,Dattilo2014,Ludka2015}, however little is known about the combined effects of partner identity, directional partner turnover, and temporal stochasticity, particularly because long-term demographic studies on ant-plant interactions are exceedingly rare \cite{Donald2022,Moraes2009}. 

Long-term demographic data are essential to understanding the fitness consequences of multi-species mutualisms for several reasons. 
...
Specifically, we asked: What are the vital rate effects of association with alternative partners and how do these effects fluctuate across years? 
What are the frequency and direction of partner turnover across the plant life cycle?
What is the net effect of partner diversity on plant fitness, and what mechanism(s) explain(s) this effect?
...
We then built a stochastic, multi-state integral projection model (IPM) which combines diverse effects on vital rates and pathways of partner turnover. 
...
Two additional 30 $\times$ 30-meter plots were added in 2011, and this group of eight plots (mean $\pm$ SD tree cholla per plot: 87.30 $\pm$ 18.28) was censused annually through 2023 (with the exception of 2020 due to the pandemic shutdown).
...
We measured reproduction by counting flowerbuds, and in most years we distinguished between flowerbuds that were viable versus aborted (aborted flowerbuds are distinguishable by dry and withered true leaves and petals). 
...
Each year, the field team searched exhaustively for new recruits by walking slowly in a standard formation across the plot area, and these were added to the census (cactus seedlings are small ($<2 cm$ in height) and cryptic so it is likely that some are missed even with a standardized search protocol). 
...
The biological ``story'' of the model is as follows: 
...
The location parameter of the skew-normal is not the mean, but the mean can be derived as $\hat{G} + (\sigma \alpha / \sqrt{1+\alpha^2}) \sqrt{2/\pi}$.
The year- and ant-specific random effect $u$ (which parameterizes the $\pmb{\tau}^G_{a}$ vectors) and plot-specific random effect $w$ are normally distributed with variances $\sigma^2_{year}$ and $\sigma^2_{plot}$, respectively (note that all ant states are assumed to share the same inter-annual variance $\sigma^2_{year}$, even as they are allowed to fluctuate independently). 
...
All of these parameters are assumed to be temporally constant.
...
$$log(\lambda_S) = E[log(N_{t+1} / N_t)]$$ \citep{Rees2009}.
...
From the full version of the model (described above) corresponding to the observed assemblage of partners and observed frequencies of partner transition, we created alternative model versions corresponding to all eight ``counter-factual'' scenarios of diversity and composition: no ant partners (complete vacancy); one ant partner (\textit{C. opuntiae} only, \textit{L. apiculatum} only, Other only); two partners (all pairwise combinations of \textit{C. opuntiae}, \textit{L. apiculatum}, and Other); and three partners (observed scenario of all ant states).
...
While technically such plants must exist in the model (because all ant states share the same $L$ and $U$ size limits), they are so infrequent, as guided by our data, as to have virtually no bearing on the results.
...
In Appendix \ref{appendix:D}, we explore additional scenarios that vary the correlation of ant-specific fluctuations, to determine if contributions from the portfolio effect are constrained by high correlations seen in the long-term data (Results).
...
\begin{figure}[H]
%\includegraphics[width=\textwidth]{Figures/surv_plot.png}
\caption{}
\label{fig:Surv}
\end{figure} 
...
Furthermore, \textit{C. opuntiae}-tended plants had lower mean seeds per fruit (115.0[79.5--165.5] seeds) than vacant (147.2[114.1--189.9] seeds) or \textit{L. apiculatum}-tended plants (142.4[100.7--200.2] seeds), likely due to pollinator interference \citep{Ohm2014}. 
...
Using simulations in which all ant species were artificially forced to be equally frequent (further explained in Appendix \ref{appendix:C}), we found the same fitness patterns as described above. 
...
In Appendix \ref{appendix:D}, we show that contributions from the portfolio effect were constrained by positively correlated fluctuations between ant states (Figure \ref{fig:Annual_Ant}). 
...
This lack of evidence for a sampling effect likely stems from the strong competitive dominance and high frequency of \textit{L. apiculatum}, which restricts the population-level impacts of alternative partners. 
...
This suggests that the benefits of partner diversity are highly sensitive to the rules of partner turnover; in systems like the cholla-ant mutualism, where a single high-quality partner is numerically and competitively dominant, the potential for diversity to increase fitness via sampling is constrained.
...
Furthermore, evidence from nearly two decades of demographic data failed to support the ``portfolio effect'' hypothesis, whereby benefits of diversity manifest in the context of environmental fluctuations, because fluctuations in the demographic effects of alternative partners are too synchronized. 
...
The weak reduction in fitness at the highest partner diversity level is harder to explain, but suggests some form of opportunity cost in a more crowded partner environment. 
...
Furthermore, our simulations exploring the absence of competitive dominance (Appendix \ref{appendix:C}) suggest that the turnover dynamics of the partner guild dictate which diversity mechanisms can manifest. 
Specifically, a sampling effect emerged only when we removed the competitive hierarchy that biases transitions toward \textit{L. apiculatum}, allowing this species to dominate the cactus population. 
This suggests that when a high-quality partner is also high-frequency due to competitive dominance, there may be little benefit of adding more species, as the host is already likely to be occupied by the best available partner.
...
For example, our previous study that focused only on reproductive vital rates suggested that \textit{C. opuntiae} has overall parasitic fitness effects because activity of this species within tree cholla flowers can deter pollinators and reduce seed set (Figure \ref{app:seeds}) \citep{Ohm2014}.
...
Future work explicitly incorporating ant-herbivore-plant interactions in the demographic models could more directly quantify how variation in ant species composition mediates herbivore pressure and plant fitness. 
...
This study highlights that while partners within a mutualistic guild can be ecologically different, they may still be interchangeable in their net effects on fitness of a shared mutualist based on where their effects occur in the life cycle, highlighting the importance of a demographic perspective in understanding the fitness consequences of mutualisms. 
...
Financial support for this work came from the Sevilleta Long-Term Ecological Research Program (NSF DEB awards 1655499, 1748133, and 2425290).
...
We also thank three reviewers for thoughtful comments and suggestions on earlier versions of this manuscript.

\section*{Author Contribution Statement}
A.M.C. and T.E.X.M. conceptualized the project and designed the analytical methods. T.E.X.M. initiated the long-term study. A.M.C. wrote the simulation code, analyzed and visualized data, analyzed the models, and wrote the original draft. 
A.M.C. and T.E.X.M. obtained funding, collected data, and participated in review and editing of the manuscript.
...
An updated data package, including recent years, is presently under review and we expect will be available in early 2026. 
For now, all of our data and code are available in our public zenodo repo: \url{https://zenodo.org/records/20030584}.
...
alt. text: A multipaneled figure. The first four panels are line plots wit 95\% confidence intervals overlaid. Each panel shows the probability of growing from one size to the next based on ant partner identity. The fifth panel shows all four of these mean lines overlaid. 

\bigskip

\textbf{Figure 2} Probability of survival in relation to size and ant partner. Layout as in Fig. \ref{fig:Grow}, except here points show data as binned means, where point size is proportional to number of observations. Grey areas show the 90\% confidence interval for the mean.

alt. text: A multipaneled figure. The first four panels are line plots wit 95\% confidence intervals overlaid. Each panel shows the probability of surviving from one year to the next based on size and ant partner identity. The fifth panel shows all four of these mean lines overlaid. 

\bigskip

\textbf{Figure 3} Flowerbud viability in relation to ant partner status. Points show observed data (fraction of initiated buds that were viable). Points and bars show the means and 95\% credible intervals for each group.

alt. text: A single paneled figure with the mean estimated flowers produced by size. The 95\% confidence interval and real data are overlaid.
...
alt. text: A multipaneled figure. The top 3 panels show correlation plots with color indicating the correlation of ant-year effects. The bottom three plots show the ant random effects across time. These bottom three are line plots.

\bigskip

\textbf{Figure 5} The probability of ant state based on plant size and previous ant state. Each panel shows size-dependent probabilities of the next ant state conditional on the previous ant state and size. Solid lines represent predictions of the multinomial statistical model and points show data binned over size intervals, where point size is proportional to the number of observations.

alt. text: A multipaneled figure. Each panel is a series of lines representing  the probability of being visited by each ant species or vacant with the data overlaid. Each panel represents a previous ant state.

\bigskip

\textbf{Figure 6} Tree cholla stochastic growth rate ($\lambda_{S}$) corresponding to number (a) or number and identity (b-c) of ant partners for synchronous and non-synchronous inter-annual fluctuations. Points and intervals show mean and 95\% credible intervals from the posterior distributions of $\lambda_{S}$. In b-c letters in the legend correspond to ant partners as follows (V = Vacant, C = \textit{C. opuntiae}, L = \textit{L. apiculatum}, O = other).

alt. text: A multipaneled figure. Each panel shows a series of means as points with 95\% confidence intervals represented as lines extending. 

\bigskip

\textbf{Figure A1} The posterior distributions of seeds produced per fruit on reproducing plants.
...
\textbf{Figure A2} The probability of a cholla plant reproducing based on its size. The green line shows the average probability, the gray surrounding area shows the 90\% confidence interval, and the black points show the actual data. 

\bigskip

\textbf{Figure A3} The number of flowers produced by a cholla plant based on its size. The green line shows the average number of flowers produced, the gray surrounding area shows the 90\% confidence interval, and the green points show the binned data.

\bigskip

\textbf{Figure B4} The proportion of reproducing plants which are visited by herbivores.
Each bar represents the observed proportion of cacti in each ant state with observed herbivores.

\bigskip

\textbf{Figure C5} Panel a) the mean estimated $\lambda_{NS}$ and $\lambda_{S}$ for different numbers of partners (0-3) for the synchronous IPM (black circle) and the non-synchronous IPM (red diamond). The lines show the posterior distribution spread of the estimated $\lambda$ values. Panels b-c) the mean estimated $\lambda_{NS}$ and $\lambda_{S}$ respectively, for each simulated combination of ant partner as the filled in circles. The lines show the posterior distribution spread of estimated $\lambda$ values. The letters in the legend correspond to what ant partners are present (V = Vacant, C = \textit{C. opuntiae}, L = \textit{L. apiculatum}, O = other).


\bigskip

\textbf{Figure C6} Panel a) the mean estimated $\lambda_{NS}$ and $\lambda_{S}$ for different numbers of partners (0-3) for the synchronous IPM (black circle) and the non-synchronous IPM (red diamond). The lines show the posterior distribution spread of the estimated $\lambda$ values. Panels b-c) the mean estimated $\lambda_{NS}$ and $\lambda_{S}$ respectively, for each simulated combination of ant partner as the filled in circles. The lines show the posterior distribution spread of estimated $\lambda$ values. The letters in the legend correspond to what ant partners are present (V = Vacant, C = \textit{C. opuntiae}, L = \textit{L. apiculatum}, O = other).

\bigskip

\textbf{Figure D7} The distributions of $\lambda_{S}^4-\lambda_{S}^0$ for the non-synchronous and synchronous models are shown in pink and green respectively. The vertical dashed line shows where the effect of partners on the fitness of the cholla is 0 (to the left the partners have a negative effect, to the right the partners are beneficial).

\bigskip

\textbf{Figure D8}  A comparison of synchronous and non-synchronous cactus responses to partner diversity across inter-annual fluctuations. 

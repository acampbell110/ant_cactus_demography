...
In total, the data set includes a total of 9,787 observations of 1141 unique individuals across \tom{19}{There are actually 15 complete transition years.} complete transition years (spanning May/June of year $t$ to May/June of year $t+1$). 
...
We fit this model to floral viability data using a binomial distribution where trials and successes are given by the total number of \tom{flower buds}{Sometimes this is flowerbuds and sometimes it is flower buds. I would tighten this up.} and the number that are viable, respectively.
...
\tom{Estimates for the number of seeds per fruit were ant specific}{I think this should be expanded. The data come from an experiment where we excluded ants, so this is a real strength (relative to the rest of the observational data) that we should not undersell.}, but the data (cite tom's data) only included \textit{L. apiculatum} and \textit{C. opuntiae} ants.
\tom{We used the vacancy estimates for plants tended by other ants. }{This is an assumption that needs to be explained or justified. Reviewers will ask if results are sensitive to this assumption.}
\tom{The rest of these parameters were not ant specific. }{Because the seed number model is ant-specific, I suggest moving it from the appendix to the main text.}
...
We fit this model to ant partner data using a multinomial distribution with a logit link function and \tom{random effects for plot and year}{As far as I can tell there are no plot or year random effects in the model you fit.}. 
...
\tom{We used $b = 200$ bins}{In your script matsize is 500.}, which we found to be sufficient for numerically stable outputs, and extended the integration limits beyond the minimum ($L$) and maximum ($U$) observed sizes to avoid unintentional eviction using the ``floor-and-ceiling'' method \cite{Williams2012}. 

...
\tom{We visually assessed parameter convergence between and within chains (Figures \ref{fig:Surv_post} -- \ref{fig:Pre_Surv_post} b) and assessed overall model fit with posterior predictive checks to examine how well the fitted model can generate simulated data similar to the real data (Figures \ref{fig:Surv_post} -- \ref{fig:Pre_Surv_post} a).}{These figures need to be labeled so that it is clear that they are in the Appendix. Am Nat's template should allow you to do this automatically.}\ali{}{They are referencing the figures, do you mean another way? I am not quite sure what this means.}
...
%\tom{We used this model to estimate probabilities which are counterfactual (such as the probability of being \textit{L. apiculatum} tended or vacant with no other options). 
...
%This assumes that competitive exclusion is the mechanism which determines the ants we see tending cacti, other potential approaches are outlined in Apendix XX.}{I would move this material to the model analysis section.}
...
\tom{Second, we re-fit the ant-dependent vital rates (survival, growth, flowerbud viability) without ant-specific year random effects, thus assuming that plants in all ant states fluctuated synchronously in response to temporal environmental variation. }{I think this is not what you are doing -- you are averaging them.}
...
\section*{Results}
\tom{}{All throughout the results section, your assertions are not sufficiently supported with evidence. This will be an important thing to improve, and it should not be hard to do.}
...
\tom{}{For growth, I suggest using one of my figure options, v1 or v2. Also, why do all of the vital rate and turnover figures have different x axis ranges? They should all show the entire size distribution.}
...
\tom{Plants tended by \textit{C. opuntiae} had a growth advantage, particularly for small plants, while plants in states of \textit{L. apiculatum}, Other ants, and vacancy had indistinguishable growth trajectories (Figure \ref{fig:Grow}).}{You cannot actually see this in the figure.}
...
Similarly, ant visitation enhanced cactus survival, and ant partner identity had \tom{a significant impact}{What is the basis for this being ``significant''?} on survival for smaller plants (Figure \ref{fig:Surv}).
...
Ant partners influence the mean viability rate of flowers, with \textit{L. apiculatum}-tended plants experiencing the highest mean viability rate (86\%, Figure \ref{fig:Viab}b), followed by vacant and Other tended plants (at 60-61\%, Figure \ref{fig:Viab}b,c), \tom{and \textit{C. opuntiae} tended plants had the lowest floral viability rate (57\%, Figure \ref{fig:Viab}d).}{This contradicts the first sentence of the paragraph. This figure legend needs to be updated.}
\tom{Plants tended by \textit{L. apiculatum} had floral viability advantages, while plants in states of \textit{C. opuntiae}, Other ants, and vacancy had very similar floral viability rates. }{This sentence is restating the previous one.}
...
We analyzed the \tom{correlation coefficients}{You are not actually providing any correlation coefficients.} of all models which included ant state as a predictor and found that the annually varying effects of each ant on survival were the least correlated, and that the effects on growth were the most correlated. 
\tom{Low correlation indicates synchronicity}{This is not correct.}, which is necessary for portfolio effect to occur because a central thesis of this effect is that partners must react differently to temporal environmental stochasticity.
Across growth, survival, and viability models, the annually varying random effects of Other ants were the most asynchronous (the least correlated with the effects of other partners), while vacant random effects were the most synchronous. 
This variety of synchronicity across ant states and vital rates indicates there is potential for portfolio effect as many of the ants effects revealed \tom{low synchronicity}{You have not actualy quantified what you mean by low synchronicity.}, particularly in the survival model. 
...
\tom{We found that 40\% of plants experienced an ant state transition on average}{I am not sure what exactly this is quantifying -- 40\% of individuals in the long-term data had at least one ant transition, is that right? I would have expected higher.}, with very distinct size-dependence and directional patterns (Figure \ref{fig:Ant_Transition}). 
...
Across all 1+ partner scenarios, \tom{we are 100\% confident}{I know what you are saying here but it would be better to rephrase this in the language of the contrasts described in the methods.} that any scenario of ant visitation elevates fitness. 
Furthermore, we find no benefits of patner diversity here, with the fitness of the cholla with 1, 2, and 3 partners roughly equivalent (Figure \tom{\ref{fig:LambdaMeans}).}{All multi-panel figures should be labeled with letters, and they should be referenced specifically. Here you are actually referencing the third panel, not the whole figure. I suggest re-ordering to make this panel first, since you describe it first. I also thought we decided to use the uncertainty version of this figure.}
...
\tom{We found that there was little benefit of any specific partner over another.
The identity and number of partners does not appear to significantly affect the fitness of the cacti. 
This indicates that while each of these partners are beneficial, there does not appear to be a significant benefit of partner diversity within this system.}{You are not referencing any figures or providing any results in the text, so there is nothing backing up your assertions here. The methods does a nice job of describing the contrasts that are the focus of the analysis but you do not ever provide those results here. You never address the apparent reduction in fitness at the full partner scenario.} 
...
We found no evidence of portfolio effect, meaning the presence of \tom{multiple partners did not buffer against the potentially negative effects of annual fluctuations}{The portfoilio effect is not about buffering negative effects of fluctuations.}.
The effect of all ant partners can be measured as $\lambda_{All} - \lambda_{Vacant}$ (Figure \ref{fig:Portfolio}).
We are are 98\% confident that when all ants are present the cholla experience higher fitness than when no ants are present according to both the synchronized (S) and non-synchronized (NS) model scenarios. 
...
The results of our heirarchical models revealed that different ant partners had different effects on vital rates, with \textit{C. opuniae} tended plants \tom{experiencing advantages in growth}{I don't think this is true if you look at my v1 figure.} and survival when small, and \textit{L. apiculatum}-tended plants experiencing floral viability advantages.
...
\tom{Previous studies have reported complementarity \citep{Palmer2010,Rezende2007,Hooper2005,Cardinale2007,Afkhami2021} while we found that there are no benefits of diversity in our system. }{This senetence cites mutualism and non-mutualism studies. It is important that your discussion be focused on effects of partner diversity in mutualisms -- that is the frame of reference against which you are interpreting your results.}
...
Differences between alternative partners were most pronounced either in reproductive rates (Figure \ref{}), which contribute relatively weakly to fitness (because a long reproductive lifespan overrides individual reproductive bouts), or in survival at small sizes, where mortality risk was high (Figure \ref{}) and the probability of ant tending was low (Figure \ref{}). 
...
\tom{We found that each ant partner had different effects on different vital rates across time, indicating they exhibit different reactions to varying environments, leading us to believe that portfolio effect could be possible in this system. 
...
We found the benefits were not stronger across time when the ants were allowed to vary naturally than when they were forced to react synchronously, indicating that portfolio effect does not explain the benefits of diversity in our system.}{This is restating methods and results. Make this more interpretive.}
\tom{Other portfolio effect studies \citep{Lazaro2022,Tornos2023} found }{??}
\tom{One study \cite{dallas2022temporal} found that while portfolio effect was easy to show in theoretical models, it is often very weak or nonexistent in empirical data across many systems. 
This indicates that it may be very difficult to detect, disguised by different mechanisms, or uncommon in nature.}{I like this point, and this is a good example of a discussion paragraph zooming out with a final sentence that makes a larger point.}
...
\tom{}{I would cut this and the next paragraph (``what remains now'') and replace them with a summary of how the results are robust to different assumptions about whether or not turnover patterns are driven by competition, and what we know and don't know about the mechanisms of turnover. You have some of that here but it can come through clearer.}
...
% rephrase this to be more relevant
...
\tom{The long term data set we used gave us the unique ability to consider the combined effects of partner identity, partner turnover, and temporal stochasticity.}{This is a great message to emphasize.}
...
\tom{In the same way that different species of ants likely react to temporal environmental variation differently, the same can be said for herbivores \citep{Thibaut2012}.
...
}{This confusing to me. Herbivores ARE the portfolio effect. We are not modeling ants becomes more or less abundant, we are modeling their demographic effects fluctuating, which could only be due to herbivores fluctuating.}
...
\tom{In the future, it would be interesting to verify the causation of our results through experimental manipulations.}{Ali, we have done these! I think this paragraph needs to be re-writted to acknowledge that, because it gives us a lot more confidence in interpreting the observational trends.}
...
\tom{Another limitation of our study is that we are simulating populations through an IPM to estimate fitness, meaning we have to extrapolate beyond observed interaction.
...
The many years of data and 10467 unique plant-year observations included in our analysis allow us to have relatively high confidence in our outputs despite extrapolations.}{I am not convinced this paragraph is necessary. The topic of creating ``counterfactual'' scenarios could be incorporated into the paragraph I am suggesting above, intepreting the different scenarios of turnover.}

\tom{}{I strongly suggest a conclusion paragraph that brings the reader back to the big picture, what we learn from this study in the broadest sense, and why it matters. Is this how mutualisms work generally? How commonly and under what conditions should we expect diversity to not matter, as it appears not to matter here?}


\section*{Acknowledgments}
\tom{This should be drafted.}{}
%%%%%%%%%%%%%%%%%%%%%

...
The survival model ($S(i,x)$) estimates the probability of survival from year $t$ to year $t1$, with fixed effects of the previous size of the cholla and ant partner and random effects of plot and year, using a Bernoulli distribution:
$$y^S ~ Bern(\hat{S})$$
$$logit(\hat{S} = \beta_{0}^{S}) + \beta_{1}^{S} \times x + \beta_{2}^{S} \times i + \beta_{3}^{S} \times x \times i + u + w$$
where $y_S$ is the probability of survival in year $t1$, $x$ is the size of the plant in the previous year $t$, $i$ is the ant partner of the plant in year $t$, $u ~ N(0,\sigma_{yr}^{2})$ is the year random effect with year specific variance of $\sigma_{yr}$, and $w ~ N(0,\sigma_{plot}^{2})$ is the plot random effect with plot specific variance of $\sigma_{plot}$.
...
The reproduction model ($P(y)$) estimates the probability of reproducing each year, with fixed effects for the size and random effects of plot and year, using a Bernoulli distribution:
$$y^P ~ Bern(\hat{P})$$
$$logit(\hat{P}) = \beta_{0}^{P} + \beta_{1}^{P} \times y + u + w$$
where $y^P$ is the probability of reproducing in year $t1$, $y$ is the size of the plant in the year $t1$, $u ~ N(0,\sigma_{yr}^{2})$ is the year random effect with year specific variance of $\sigma_{yr}$, and $w ~ N(0,\sigma_{plot}^{2})$ is the plot random effect with plot specific variance of $\sigma_{plot}$.
...
$$y^{F} ~ Negative Binom(\hat{F})$$
$$log(\hat{F}) = \beta_{0}^{F} + \beta_{1}^{F} \times x + u + w$$
where $y^{F}$ is the number of flowers produced by a plant in year $t1$, $x$ is the size of the plant in year $t$, $u ~ N(0,\sigma_{yr}^{2})$ is the year random effect with year specific variance of $\sigma_{yr}$, and $w ~ N(0,\sigma_{plot}^{2})$ is the plot random effect with plot specific variance of $\sigma_{plot}$.
...
where $y^{V}$ is the proportion of flowerbuds produced that remain viable in year $t1$, $i$ is the ant partner of the plant in year $t$,  $u ~ N(0,\sigma_{yr}^{2})$ is the year random effect with year specific variance of $\sigma_{yr}$, and $w ~ N(0,\sigma_{plot}^{2})$ is the plot random effect with plot specific variance of $\sigma_{plot}$.

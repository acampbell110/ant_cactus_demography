...
\paragraph{Growth}
The growth sub-model ($G(x',x,a;\pmb{\tau^G_{a}})$) gives the probability of future size given \tom{the fixed effects of previous size $x$ and previous ant partner $a$ and random effects of plot $w$ and year $u$. }{You need to go a little farther than this, because the fixed effects are arranged as a second-order polynomial with size-ant interactions. That needs to be explained and the choice of polynomial should be justified. You also do not describe the size-dependence in variance and kurtosis. Also, I would subscript the $u$ as $u_{a}$, then explain what that means. The $u_{a}$ is important to describe in a biologically meaningful way because this is how the portfolio effect is tested.  Finally, notice that the tau vector does not actually appear in the model. This is a problem. You can solve it by changing u to tau, or vice versa.}
We fit this model to size data at the end of the transition year $y^G$ using the location-scale parameterization of the \tom{student $t$ distribution}{Don't kill me but I think we should either figure out a skewed t or go back to the skew normal. But just to comment on the text as is, you need to describe the three parameters of the t (e.g., G-hat is the mean) in order for the equations to make sense.}, because in preliminary analyses we found that size transition data were more fat-tailed than a Gaussian distribution could accommodate. 
...
$$\hat{\nu} = \beta_{0}^{\nu} + \beta_{1}^{\nu} \times x $$
\tom{}{I think the equations will be more readable if you do not use the $\times$ symbol for multiplication.}
where \tom{$u \sim N(0,\sigma_{yr \times a}^{2})$ is the year random effect with year specific variance of $\sigma_{yr}$ with ant $a$ in year $t$}{This description is not correct. The variance is not ant-specific, the deviates are.}, and $w \sim N(0,\sigma_{plot}^{2})$ is the plot random effect with plot specific variance of $\sigma_{plot}$. 
\tom{Ants are included as predictors here because ant partners defend cholla from herbivores, preventing vegetative tissue loss.}{I think this is unnecessary since you provide the rationale for ant effects in the beginning of this section.}

\paragraph{Survival}
The survival model ($S(a,x;\pmb{\tau_{a}^{S}})$) estimates the \tom{probability of survival $y^S$}{$y^S$ is not the probability of survival} from year $t$ to year $t+1$, with fixed effects of the previous size of the cholla $x$ and ant partner $a$ in year $t$ and random effects of plot $w$ and year $u$, using a Bernoulli distribution:
$$y^S \sim Bern(\hat{S})$$
$$logit(\hat{S} = \beta_{0}^{S}) + \beta_{1}^{S} \times x + \beta_{2}^{S} \times a + \beta_{3}^{S} \times x \times a + u + w$$
\tom{where $u \sim N(0,\sigma_{yr \times a}^{2})$ is the year random effect with year specific variance of $\sigma_{yr}$ and ant state $a$ in year $t$, and $w \sim N(0,\sigma_{plot}^{2})$ is the plot random effect with plot specific variance of $\sigma_{plot}$.}{Much of this is redundant with the growth model, and likely redundant with models below. Think about efficient ways to economize text so that you are not re-explaining the same things across sub-models.} 

\paragraph{Reproduction}
The reproduction model ($P(x';\pmb{\tau^{P}})$) estimates the \tom{probability of reproducing $y^P$}{Same comment: $y^P$ is not the probability of reproducing.} in year $t+1$, with fixed effects for the size $x'$ in year $t+1$ and random effects of plot $w$ and year $u$, using a Bernoulli distribution:
$$y^P \sim Bern(\hat{P})$$
...
\paragraph{Flowers}
The total flowers model ($F(x';\pmb{\tau^{F}})$) estimates the total flowers produced by a plant $y^F$ in year $t+1$, with fixed effects of size $x'$ in year $t+1$ and random effects of plot $w$ and year $u$, using a zero-truncated negative binomial distribution:
...
For each model, we obtained \tom{3}{Spell out numbers less than ten.} chains of 10,000 iterations, each with randomly chosen initial conditions. 
The first 1,500 iterations were discarded. 
We did not thin the chains, thus all samples were retained following burn-in. 
We assessed parameter convergence between and within chains (\tom{Fig.}{reference figure}). 
To assess the overall model fit we carried out posterior predictive checks to examine how well the fitted model can generate simulated data similar to the real data (\tom{Fig.}{reference figure}). 
\tom{All estimated parameters are available in the data. }{What does this mean?}
\tom{Data and code for all vital rate models is included in the supplemental information.}{I do not know what this refers to. Am Nat (and most journals) requires a data accessibility statement that will need to include details of data and code availability.}

\subsection*{IPM Analysis}
\tom{}{I think this section needs to start with an overview paragraph descibing in plain English what you are trying to do with the fitted model, before getting into any technical details.}
Analyzing an IPM requires discretizing the continuous IPM kernel into an approximating matrix \tom{to calculate the dominant eigenvalue}{The dominant eigenvalue is but one of many, MANY forms of ``analysis'' of an IPM. I would cut this bit that I highlighted.}. 
Size variable $x$ is discretized into $b$ bins, resulting in a $b \times b$ matrix.
In our model there is additional complexity in the form of transitions between ant partners, leading to a matrix size of \tom{$m b \times m b$ where $m$ is the number of unique combinations of $a$ and $a'$ (how many possible ant transitions there are).}{I think what you call $m$ here is actually just $A$, the total number of ant states.} 
Our model also has two additional discrete states (year one and year two seed banks) leading to a final matrix size of $m(b+2) \times m(b+2)$.
\tom{We used $b = 200$ bins.}{It is also useful to briefly acknowledge \emph{why} 200.}
\tom{We extend the integration limits $L$ and $U$ to avoid unintentional eviction \cite{Williams2012}.}{You have not said how you estimate $L$ and $U$ from the data.}
...
\tom{In a deterministic bayesian IPM,}{I thought you were only using the two types of stochastic models (and I have not read ahead).} we create \tom{1,000 discrete kernels with a unique set of parameters}{This should explicitly refer to samples from the posterior distribution of parameter values} from our bayesian statistical models to estimate 1,000 $\lambda$ values as a distribution. 
\tom{In a deterministic version of our IPM}{At this point, it is not clear to readers how many ``versions'' you have and why, which is why an overview paragraph providing a ``roadmap'' will be an important addition.}, \tom{this process is repeated separately for every combination of ant partners: complete vacancy; \textit{C. opuntiae} and vacancy; other and vacancy; \textit{L. apiculatum}, \textit{C. opuntiae}, and vacancy, \textit{L. apiculatum}, other, and vacancy; \textit{C. opuntiae}, other, and vacancy; and all ant partners and vacancy.}{Rationale for why single ant scenarios are coupled with vacancy should be better developed.}
In order to calculate stochastic $\lambda$ distributions for each ant scenario, \tom{every one of the 1,000 parameter-iteration associated $\lambda$s comes from the mean of 5,000 year-specific estimations each associated with a year random effech which was randomly selected from the 18 years of data. }{Even having worked on this with you for a long time, I cannot follow what you are saying here. This will need careful editing so that the methods and rationale for those methods (just as important!) are crystal-clear.}

\tom{We compare the distributions of $\lambda$ across each combination of ant partners to whether sampling effect or complementarity is at play in the system.}{I do not think you have sufficiently developed the logic for how these comparisons inform hypotheses about different mechanisms. Also, and this is a matter of taste, I find the language ``at play in the system'' to be not very scientific and would prefer to phrase this more precisely.}
To compare the distributions of two hypothetical \tom{populations, A and B}{It is not clear what this toy example stands in for in the real model.}, we subtract the vector of 1,000 $\lambda_A$ estimations of one population from the 1,000 $\lambda_B$ estimations of the other ($\lambda_A - \lambda_B$).
\tom{If the average of these differences is positive, population A has a higher $\lambda$ than B, if the average is 0, they are equal, and if the average is negative, population B has a higher $\lambda$ than A.}{I think this could be edited to more effectively communicate what is gained from these contrasts. The average will never be exactly zero. The important element, IMO, is that we gain a probabilistic estimate of which combinations have higher or lower fitness, given the data.} 

\tom{To determine if sampling effect is at play in a system}{Ditto previous comment re: ``at play''. But it is also important to recognize, as I have emphasized before, that these are mechanisms that could explain positive diversity effects. Your writing here presumes a positive diversity effect.}, we must first determine if there is a "best" partner, by determining which single ant association is correlated with the highest \tom{$\lambda$}{Here and elsewhere, it is important to use correct notation for fitness. $\lambda$ is population mean fitness in a constant environment. $\lambda_{S}$ is population mean fitness in a stochastic environment. If you are talking about stochastic environments, you should be using $\lambda_{S}$. I also think it would be helpful to notate either $\lambda$ with ant partner, e.g. $\lambda^{C}$ is fitness corresponding to exclusive association with \textit{Crematogsaster}. I think that would help communicate methods.} estimation.
\tom{We can do this by comparing the distributions of each $\lambda$ and finding the one which is larger than all others.}{Distributions are not larger than one another. They have properties -- moments -- that can differ, like the mean or standard deviation. Be more precise here.}
Then we \tom{must show}{I don't like ``must show'' -- sounds like we are hunting for something.} that the $\lambda$ of a population with all possible partners is equal to that of $\lambda$ of a population with only the "best" partner, by comparing the distributions as described earlier.
...
\tom{To do this we parameterized a new set of bayesian statistical models in which year random effects were not ant specific, meaning the effects of ant partners were not able to covary with year effects (the scenario in which all ant partners must respond to annual variation the same way and portfolio effect is not possible).
The new estimations of year random effects are $u \sim N(0,\sigma_{yr}^2)$ with year specific variance of $\sigma_{yr}$.}{This needs to be explained earlier, in the statistical estimation sections, so that this section can focus on teasing out mechanisms from the model. I also think you need to better explain what the random year effects represent, biologically. We do not have any data on how the ants are actually fluctuating, but we do know something about how the effects of the ants on plants fluctuates. That is a critical nuance and it does not currently come through. I am also wondering whether it would be preferable to have the $a$ coefficients temporally fluctuate rather than making the $u$ ant-specific. Or are these effectively the same thing? (we could probably figure it out on a whiteboard)} 
With this verision of the IPM (now referred to as the stochastic null IPM) \tom{we followed the same approach to calculate distributions of stochastic $\lambda$s for each ant scenario.}{It is still not clear if what you described above was based on deterministic or stochastic $\lambda$.}
...
\tom{}{General comment: the methods for this analysis are a little complicated, which is an inevitable consequence of asking a complex question. In these cases, a writer needs to really step up to communicate something complicated in a way that is sufficiently simple that it can be easily understood and, ideally, replicated. I think with focused effort you can significantly improve this section. Again: put yourself in the shoes of the reader and think about what they need from you in order to ``get it''.}  
  
\section*{Results}
\tom{}{I strongly suggest re-structuring the Results section such that each sub-section is titled with a question from the end of the Intro, and the content of the sub-section answers that question.}
\subsection*{IPM Analysis}
...
\tom{We found that populations of tree cholla which have no partners have a mean fitness of $\lambda = 0.9861$ \ref{}.
...
Populations with all ant partners present have a mean fitness of $\lambda = 0.9940$.}{This is not an effective use of Results text. You are just reporting values -- this is why we have figures!}
...
We \tom{used bayesian modeling}{I recommend less emphasis on this. It is not the only way to get a distribution of lambda values, I don't think it is the most important thing to emphasize about the study, and you risk rankling reviewers who might think you are trying to sound impressive with ``fancy statistics''.}, so each of these $\lambda$ estimations has a distribution rather than a single estimate.
...
\tom{We are between 82\% to 100\% confident that having any ant parnter leads to a higher $\lambda$ than having no partners.}{Where can readers see this result? You say there are distributions lambdas and differences but you do not show them, and that will need to be remedied. ALso, in terms of fitness contrasts, I think it is more important to start with the Vacant -- All Ant contrast. ``all ants'' is our best representation of what is happening in nature, so it is relevant to first ask if there is evidence that these interactions, collectively, are beneficial, and then use the hypothetical scenarios to unpack how those benefits arise. }

\begin{figure}[H]
	\includegraphics[width=0.95\linewidth]{Figures/LambdaMeans.png}
	\caption{THIS FIGURE HAS NO LEGEND. Are you sure this is not deterministic and stochastic? I am surprised that both stochastic models would be this different.}
\end{figure}
...
\tom{These results indicate that sampling effect, not complementarity, is at play in this system.}{I suspect we may get some push-back on this interpretation, because the sampling effect is a mechanisms to explain benefits of diversity. Based on your results, there are no benefits of diversity, there is just a best partner. I am not suggesting to entirely discard the sampling effect idea, but it is important to recognize that when you discuss these mechanisms, they are mechanisms for positive diversity effects. So the absence of any diversity effect seems like an important result to contextualize how you interpret evidence for a sampling effect.}


%%%% Stochastic null follows similar patterns but dampened
\tom{}{Throughout this paragraph and much of the results section, you are not explicitly referencing figures, so it is not always clear what is the basis for some of your assertions. Also, the figures need legends.}
...
We found that both the stochastic model and stochastic null model resulted in higher fitness when all ants are present compared to no partners \tom{94\% of the time}{This is not a measure of time, it is a measure of confidence based on the posterior probability of the parameters given the data.}. 
These results differed in the magnitude of fitness boost received from partners.
...
\tom{We are 52\% confident that the difference is greater when inter-annual effects were not ant dependent (in the stochastic null model).}{If I understand this correctly (and I am not sure because you are not showing the results) then it is equally likely that fitness is either higher or lower when ant effects do or do not fluctuate synchronously -- this tells me there is no evidence for a portfolio effect.}
\tom{This indicates that portfolio effect is at play, if only weakly evident. }{This is not a very scientific statement.}

\subsection*{Statistical Modeling}
\tom{}{In my opinion it does not make sense to have the statistical modeling results follow the IPM results. But I also think the results would benefit from restructuring around the motivating questions (which would put vital rates first).}
...
\begin{figure}[H]
	...
	\label{fig:growth}
\end{figure}
\tom{}{Here and elsewhere, begin paragraphs with the main result that you want readers to see. In this case I think you want a topic sentence to emphasize that there is evidence that ant visitation enhances cactus growth, partner identity can influence the growth trajectory. That is a simple statement but it captures the essential reason why you did this analysis in the first place. That connection to your motivating questions is often lacking.}Tree cholla experience positive mean growth rates across all partners at small to medium sizes with the highest chances of shrinking occurring at the largest sizes \tom{\ref{fig:growth}e}{When you reference figures like this the ref function only returns the number, so you need to type ``Figure''.}.
\tom{The mean growth rate expected for plants that are 10 $cm^3$ or smaller in year t is 10.01 $cm^3/yr$, 4.65 $cm^3/yr$, 7.44 $cm^3/yr$, and 5.14 $cm^3/yr$ when tended by \textit{C. opuntiae}, \textit{L. apiculatum}, other ants, or no ants respectively. 
...
The mean growth rate expected for plants that are larger than 150 $cm^3$ in year t is 1.28 $cm^3/yr$, 1.21 $cm^3/yr$, 1.29 $cm^3/yr$, and 1.10 $cm^3/yr$ when tended by \textit{C. opuntiae}, \textit{L. apiculatum}, other ants, or no ants respectively. }{Similar to my comment above, this type of recapitulation of the figure is not a very effective use of space.}
...
\tom{We are 89\%, 88\%, and 70\% confident that plants tended by \textit{C. opuntiae} ants experience higher mean growth rates across sizes than plants tended by no parnters, \textit{L. spiculatum} ants, or other ants respectively.
We are 89\%, 65\%, and 94\% confident that plants with no partners experience lower mean growth rates across all sizes than plants tended by \textit{C. opuntiae}, \textit{L. apiculatum}, or other ants respectively.}{The empirical basis for these assertions is not clear. I am also a little confused because the text above makes it sounds like the ranks are dependent on size.}
...
\tom{}{This section is hard to read, and I think it needs to be restructured in a way that is guided by what you want readers to know as it relates to your questions. You provide lots and lots of statistics and contrasts without much context for why these matter and how they should be interpreted. I suggest you think about the patterns that emerge for you in the figure, and re-write this section with that in mind. I do not think this is much value in breaking it down into the four ant states. You should be able to describe transition dynamics in an engaging and succinct way in just a paragraph or two.}
\begin{figure}
...
\textit{L. apiculatum} ants are the most likely \tom{tender}{Not sure I would use this as a noun.} in year $t+1$ for all large plants that were not previously tended by \textit{C. opuntiae} ants in year $t$, while \textit{C. opuntiae ants} tended plants in year $t$ are most likely to remain tended by \textit{C. opuntiae} ants in year $t+1$.
\tom{We are 93\%, 89\%, 86\%, and 93\% confident that plants that are smaller than 150 $cm^3$ in year $t$ will be vacant in year $t+1$ when they were tended by no partners, \textit{C. opuntiae}, \textit{L. apiculatum}, or other ants respectively in year $t$.}{Statements like these do not feel strongly motivated, and I am not sure what you want me to know as it relates to the motivating questions.}

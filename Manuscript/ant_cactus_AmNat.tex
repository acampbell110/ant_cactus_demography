...
Out study area in the Los Pi$\tilde{n}$os mountains \tom{()}{Give lat-long} is characterized by steep, rocky slopes, and perennial vegetation including grasses (\textit{Bouteloua eriopoda} and \textit{B. gracilis}), yuccas, cacti, and junipers. 
...
In addition to missing the year 2020, there are gaps in the time series where we switched plots or plants (and thus broke up transition years growth/survival and partner turnover) or where we did not distinguish between viable and aborted flower buds \tom{(Table \ref{})}{Missing table reference. There is no table in the supplement, but there is an empty heading for ``Table''}. 
...
\tom{This function includes inter-annual variability through year-specific intercepts which are consistent across initial ant states ($\pmb{\tau}^\rho$).}{Confirming that this model includes random year effects?}
...
We fit this model to floral viability data using a \tom{binomial}{For the inevitable revision, I would like to try beta-binomial.} distribution where trials and successes are given by the total number of flowerbuds and the number that are viable, respectively.
...
$$log(\lambda_S) = E[log(\frac{N_{t+1}}{N_{t}})]$$ \tom{\citep{Mark2009}}{Mark is Mark Rees' first name. This bibtex entry needs to be updated.}.
...
\tom{Similarly, for plants which were large enough to have ant visitors, visitation enhanced cactus survival (Figure \ref{fig:Surv}). }{Stay consistent with use of dashed vs solid lines.}
...
\tom{Mean viability rates spanned 40--92\% (Figure \ref{fig:Viab}).}{I am not sure where this range comes from. The group means look like they span about 55--88.}
...
Furthermore, \textit{C. opuntiae}-tended plants had fewer seeds per fruit (115.0[79.5--165.5] seeds) than vacant (147.2[114.1--189.9] seeds) or \textit{L. apiculatum}-tended plants (142.4[100.7--200.2] seeds) (Figure S\ref{app:AppA_Seeds_Per_Fruit}), likely due to pollinator interference \citep{Ohm2014}.
...
Dallas et al. \citeyear{dallas2022temporal} found that while the portfolio effect was easy to show in theoretical models, it is often \tom{very weak or nonexistent in empirical data across many systems. }{Does this mean it is actually weak/nonexistent, or just difficult to detect? (I don't know the paper.)}
...
The data that support the findings of this study are openly available as a \href{https://portal.edirepository.org/nis/mapbrowse?packageid=knb-lter-sev.323.1}{data package} on the Environmental Data Initiative website, package id: \tom{knb-lter-sev.323.1}{UPdate this so that it's a normal author-year citation. And update the data package!}

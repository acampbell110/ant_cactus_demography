...
\response{I removed this sentence based on their recommendations so I am not sure what I should say here... ********* Tom: I will check the manuscript text, but probably simplest to change the way this paper gets cited so that we are not asserting that mutualism is more context-dependent.}
...
\response{The simplest response we can offer here is that the small plant data are \emph{effectively} not extrapolated, so the expected performance of tiny ant-tended plants has little bearing on our results. 
	The more complete, technical response is that some amount of extrapolation is an inevitable feature of the Integral Projection Model framework. 
	...
	Tiny, ant-tended plants do exist in the model as a possible state combination, because they must -- but these are at such low frequency (as informed by the data) that the model is effectively insensitive to what we assume about their demographic performance.
	We have tried to better communicate this nuance in our presentation of the model (line \ref{}).}
...
How relevant?}
\response{We have edited this sentence to be more specific about ``applied settings'' we referenced previously. 
For example, prior work has examine how diversity of pollinator assemblages affects productivity of crop plants (see line \ref{}). }

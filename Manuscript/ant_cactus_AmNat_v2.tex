...
Defensive ant-plant mutualisms -- where plants provide food and/or housing to ants that in turn defend them from enemies -- are widespread interactions that offer valuable model systems for understanding the ecology and evolution of mutualism \citep{Bronstein1998, Bronstein2006}. 
...
In our study region in central New Mexico, US, tree cholla are tended by two common ant species (\textit{Liometopum apiculatum} and \textit{Crematogaster opuntiae}) and several additional rarer species, all of which collect EFN during foraging visits but their colonies are ground-nesting and not housed by the plants. 
These ant species locally co-occur but individual plants are typically tended by only one species that patrols the plant around-the-clock and maintains control of the plant's nectar resources, typically for an entire growing season \citep{Ohm2014, Donald2022}. 
...
Furthermore, to our knowledge no previous study has incorporated inter-annual stochasticity into models of ant-plant dynamics, which limits our understanding of diversity benefits that may arise through the portfolio effect. 
...
We used this observational data set of ant-plant associations, contextualized by previous ant exclusion experiments, to ask whether and through which mechanism(s) partner diversity affects the fitness benefits of ant visitation for the focal plant partner. 
...
\subsubsection*{SIMULATION IDK WHAT TO CALL THIS RN }
Using an alternative transition model to parameterize the IPM, we conducted simulation experiments to determine if the sheer number of \textit{L. apiculatum} ants might be driving the outcomes of our partner diversity experiments. 
We followed the same protocol as the partner diversity experiments, except the transition model equalized the frequency of each ant partner.
We still preserved the observed pattern of size-dependent vacancy/occupancy, but occupancy was manipulated to be equally likely for all partner identities. 
...
The results of each of these as well as the posterior predictive checks are all included in the Apendix XX.
...
Despite the positive effects of ant visitation in any combination of partners, when identity of partners is considered, we found that there is an opportunity cost to partner diversity.
There is no fitness scenario better than \textit{L. apiculatum} ants alone.
All other scenarios lead to diminished fitness of tree cholla, indicating that there is not a benefit of diversity in this system.
...
Using the simulations where all ants had equal frequencies across sizes, we found that scenarios with \textit{C. opuntiae} and Other ants present experienced fitness boosts under this simulation, though not eough to offer better fitness than \textit{L. apiculatum}.
Equal probability for transitioning into any ant state meant that the numbers of these ants were boosted significantly.
...
However, our IPM results showed that we were actually observing sampling effect rather than complementarity, leaving us with questions about how these results could fit together. 
...
In addition to the competitive exclusion model and the equal probability (null) model defined and analyzed in the body of the paper, we simulated results from several other potential models. 
...
However, in the building and testing of alternative models we found that the method of ants occupying plants significantly impacts the fitness of the population.

\paragraph{Frequency Model}
The first alternative hypothesis we tested was what we called the frequncy model.
...
With this model we found that both \textit{C. opuntiae} and Other ants are parasites when they are the only partner present (Figure \ref{fig:FreqLambdaMeans}a).
This indicates that at low frequencies, these ants are not able to provide strong benefits.
Diversity is beneficial through a sampling effect in this simulation (Figure \ref{fig:FreqLambdaMeans}b).

\begin{figure}
\includegraphics[width=0.61\linewidth]{Figures/lambda_means_freq.png}
\includegraphics[width=0.39\linewidth]{Figures/Lambda_Num_Partners_freq.png}
\caption{Panel a) shows the mean values of the estimated $\lambda_{S}$ (filled in circles) for each simulated combination of ant partners. The letters above the points correspond to what ant partners are present (V = Vacant, C = \textit{C. opuntiae}, L = \textit{L. apiculatum}, O = other). Panel b) shows the mean values of the estimated $\lambda_{S}$ for }
\label{fig:FreqLambdaMeans}
\end{figure}

\paragraph{In-Between Model}
The first alternative hypothesis we tested was what we called the in-between model.
In this model rather than only competitive exclusion or frequency based occupancy, the occupancy is in-between these two models. 
We once again found that both \textit{C. opuntiae} and Other ants are parasites when they are the only partner present (Figure \ref{fig:BetweenLambdaMeans}a).
Diversity is beneficial through a sampling effect in this simulation (Figure \ref{fig:BetweenLambdaMeans}b).

\begin{figure}
\includegraphics[width=0.61\linewidth]{Figures/lambda_means_between.png}
\includegraphics[width=0.39\linewidth]{Figures/Lambda_Num_Partners_between.png}
\caption{Panel a) shows the mean values of the estimated $\lambda_{S}$ (filled in circles) for each simulated combination of ant partners. The letters above the points correspond to what ant partners are present (V = Vacant, C = \textit{C. opuntiae}, L = \textit{L. apiculatum}, O = other). Panel b) shows the mean values of the estimated $\lambda_{S}$ for }

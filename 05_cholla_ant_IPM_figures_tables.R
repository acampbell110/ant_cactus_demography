...
y_crem_subset_grow <- subset(growth_data, ant_t == "crem")
y_liom_subset_grow <- subset(growth_data, ant_t == "liom")
y_vac_subset_grow <- subset(growth_data, ant_t == "vacant")
y_other_subset_grow <- subset(growth_data, ant_t == "other")
...
# All together
plot(x = size_dummy, y = invlogit(percentiles("other",0.5)), type = "l", col = othercol, lwd = 2, ylim = c(0,1), lty = 2, xlim = c(-5,15), main = "e)                      All Ants                           ", cex.main = 2)
...
# Prev Other
plot(size_dummy, pred_other[,4], type = "l", col = vaccol,main = expression(bold(paste("b)", sep = "              ", "Prev. Other", "             "))), ylim = c(0,1), xlab = "", ylab = "",
     cex.main = 1.5, )
...
ant_freq_ts <- cactus[,c("Year_t","ant_t")]
ant_freq_ts <- drop_na(ant_freq_ts)
ant_freq_ts <- ant_freq_ts %>% group_by(Year_t,ant_t) %>% 
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n))
...
lams_comp_stoch_null <- read.csv("Model Outputs/lambda_sync_comp.csv")
lams_comp_stoch_null <- lams_comp_stoch_null[,-c(1)]
## Frequency Based
lams_freq_stoch <- read.csv("Model Outputs/lambda_freq.csv")
lams_freq_stoch <- lams_freq_stoch[,-c(1)]
## Frequency Based Null
lams_freq_stoch_null <- read.csv("Model Outputs/lambda_sync_freq.csv")
lams_freq_stoch_null <- lams_freq_stoch_null[,-c(1)]
## Equal Probability
lams_equal_stoch <- read.csv("Model Outputs/lambda_equal.csv")
lams_equal_stoch <- lams_equal_stoch[,-c(1)]
## Equal Probability Null Stochastic
lams_equal_stoch_null <- read.csv("Model Outputs/lambda_sync_equal.csv")

...
#### Growth Model ############################################################################
##############################################################################################
## Create Stan Data Crem Liom
stan_data_grow_crem_liom <- list(N_grow = nrow(growth_data_crem_liom), ## number of observations
                       vol_grow = log(growth_data_crem_liom$volume_t), ## predictors volume
                       y_grow = log(growth_data_crem_liom$volume_t1), ## response volume next year
                       ant_grow = growth_data_crem_liom$ant,## predictors ants
                       N_ant = 2, ## number of ant states
                       N_Year_grow = max(growth_data_crem_liom$year), ## number of years
                       N_Plot_grow = max(growth_data_crem_liom$Plot), ## number of plots
                       plot_grow = growth_data_crem_liom$Plot, ## predictor plots
                       year_grow = growth_data_crem_liom$year ## predictor years
) 
#Check that you are happy with the subsetting
plot(stan_data_grow_crem_liom$vol_grow, stan_data_grow_crem_liom$y_grow)
plot(log(cactus_crem_liom$volume_t), log(cactus_crem_liom$volume_t1))
## Run the Model
#Check if the model is written to the right place
#stanc("STAN Models/grow_mix_ant.stan")
fit_grow_mix_ant_crem_liom <- stan(file = "STAN Models/grow_mix_ant_crem_liom.stan", data = stan_data_grow_crem_liom, warmup = 500, iter = 1000, chains = 3, cores = 3, thin = 1)
grow_outputs_crem_liom <- rstan::extract(fit_grow_mix_ant_crem_liom, pars = c("beta0","beta1","sigma"))
grow_yrep_crem_liom <- rstan::extract(fit_grow_mix_ant_crem_liom, pars = c("y_rep"))$y_rep
write.csv(grow_outputs_crem_liom, "grow_outputs_crem_liom.csv")

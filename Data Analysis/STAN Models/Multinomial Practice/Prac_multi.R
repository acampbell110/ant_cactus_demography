...
# Generate Fake Data
generate_mnl_data <- function(N=10, C=4, beta=c(-3), alpha = c(1,2,3,4)){
  K <- length(beta)
  Y <- rep(NA, N)
  X <- list(NULL) 
  for (i in 1:N) {
...
generate_mnl_data <- function(N=1000, K=4){
  D <- 2
  Y <- rep(NA, N)
  X <- rep(NA, N) 
  Z <- rep(NA, N)
  for (i in 1:N) {
    X[i] <- rnorm(1)
    Z[i] <- sample(c(1,2,3,4), size = 1, replace = TRUE, prob = NULL)
    Y[i] <- sample(x=K, size=1) # logit formula
  }
  list(N=N, D=D, K=K, Y=Y, X=X, Z = Z)
}
d1 <- generate_mnl_data(N=1000, K=4)
str(d1)
## Run the Model
mod2 <- stan(file = "multi_2.stan",data = d1, warmup = 5, iter = 10, chains = 3, cores = 2, thin = 1)
...
mu_M <- rstan::extract(mod2, pars = c("mu"))$mu
#draw 500 random samples from the joint posterior
n_post_draws <- 5
post_draws <- sample.int(dim(mu_M)[1], n_post_draws)
#set up simulation output
y_m_sim <- matrix(NA,n_post_draws,length(X))
#loop over the posterior and generate new observations
for(i in 1:n_post_draws){
  print(i)
  ## sample survival data (bernoulli)
  y_m_sim[i,] <- rmultinom(n=c(1,2,3,4), size=1, prob = invlogit(mu_M[i,]))
}

bayesplot::ppc_dens_overlay(Y, y_m_sim)

...
parameters {
  ...
  real <lower = 0 > skew; // the skew of the distribution
}
...
//Priors
 ...
 skew ~ normal(0,100); // the skew of the distribution
 ...
 //Model
 for(i in 1:N){
 y_grow[i] ~ skew_normal(mu[i],sigma[i],skew);
 }
}
generated quantities {
  real y_rep[N] = skew_normal_rng(mu,sigma,skew);
}

...
##
#######################################################################################################
source("/Users/alicampbell/Documents/GitHub/ant_cactus_demography/Setup_Data.R")

#### Growth Model ############################################################################
##############################################################################################
## Create Stan Data Crem
stan_data_grow_crem <- list(N_grow = nrow(growth_data_crem), ## number of observations
                       vol_grow = log(growth_data_crem$volume_t), ## predictors volume
                       y_grow = log(growth_data_crem$volume_t1), ## response volume next year
                       ant_grow = growth_data_crem$ant,## predictors ants
                       N_ant = 1, ## number of ant states
                       N_Year_grow = max(growth_data_crem$year), ## number of years
                       N_Plot_grow = max(growth_data_crem$Plot), ## number of plots
                       plot_grow = growth_data_crem$Plot, ## predictor plots
                       year_grow = growth_data_crem$year ## predictor years
) 
stan_data_grow_liom <- list(N_grow = nrow(growth_data_liom), ## number of observations
                            vol_grow = log(growth_data_liom$volume_t), ## predictors volume
                            y_grow = log(growth_data_liom$volume_t1), ## response volume next year
                            ant_grow = growth_data_liom$ant,## predictors ants
                            N_ant = 1, ## number of ant states
                            N_Year_grow = max(growth_data_liom$year), ## number of years
                            N_Plot_grow = max(growth_data_liom$Plot), ## number of plots
                            plot_grow = growth_data_liom$Plot, ## predictor plots
                            year_grow = growth_data_liom$year ## predictor years
) 
stan_data_grow_other <- list(N_grow = nrow(growth_data_other), ## number of observations
                            vol_grow = log(growth_data_other$volume_t), ## predictors volume
                            y_grow = log(growth_data_other$volume_t1), ## response volume next year
                            ant_grow = growth_data_other$ant,## predictors ants
                            N_ant = 1, ## number of ant states
                            N_Year_grow = max(growth_data_other$year), ## number of years
                            N_Plot_grow = max(growth_data_other$Plot), ## number of plots
                            plot_grow = growth_data_other$Plot, ## predictor plots
                            year_grow = growth_data_other$year ## predictor years
) 
stan_data_grow_vac <- list(N_grow = nrow(growth_data_vac), ## number of observations
                            vol_grow = log(growth_data_vac$volume_t), ## predictors volume
                            y_grow = log(growth_data_vac$volume_t1), ## response volume next year
                            ant_grow = growth_data_vac$ant,## predictors ants
                            N_ant = 1, ## number of ant states
                            N_Year_grow = max(growth_data_crem$year), ## number of years
                            N_Plot_grow = max(growth_data_crem$Plot), ## number of plots
                            plot_grow = growth_data_crem$Plot, ## predictor plots
...
########################################################################################################
## Create Stan Data
stan_data_surv <- list(N_surv = nrow(survival_data_crem), ## number of observations
